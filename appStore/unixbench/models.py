...
class Unixbench(models.Model):
    ...
    single_execute_cmd = models.CharField(null=True, blank=True, max_length=255, verbose_name='执行命令')
    single_modify_parameters = models.CharField(null=True, blank=True, max_length=255, verbose_name='修改参数')
    single_Dhrystone = models.FloatField(verbose_name='single Dhrystone 2 using register variables(lps)')
    single_Double_Precision = models.FloatField(verbose_name='single Double-Precision Whetstone(MWIPS)')
    single_execl_throughput = models.FloatField(verbose_name='single Execl Throughput(lps)')
    single_file_copy_1024 = models.FloatField(verbose_name='single File Copy 1024 bufsize 2000 maxblocks(KBps)')
    single_file_copy_256 = models.FloatField(verbose_name='single File Copy 256 bufsize 500 maxblocks(KBps)')
    single_file_copy_4096 = models.FloatField(verbose_name='single File Copy 4096 bufsize 8000 maxblocks(KBps)')
    single_pipe_throughput = models.FloatField(verbose_name='single Pipe Throughput(lps)')
    single_pipe_based = models.FloatField(verbose_name='single Pipe-based Context Switching(lps)')
    single_process_creation = models.FloatField(verbose_name='single Process Creation(lps)')
    single_shell_scripts_1 = models.FloatField(verbose_name='single Shell Scripts (1 concurrent)(lpm)')
    single_shell_scripts_8 = models.FloatField(verbose_name='single Shell Scripts (8 concurrent)(lpm)')
    single_system_call_overhead = models.FloatField(verbose_name='single System Call Overhead(lps)')
    single_index_score = models.FloatField(verbose_name='single Index Score(sum)')
    single_test_time = models.DateTimeField(verbose_name="single 测试时间", default=timezone.now)
