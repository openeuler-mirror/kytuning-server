...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def merge_data(self, request, *args, **kwargs):
        ...
        if Project.objects.filter(env_id=env_id).first().merge_env_ids:
            new_env_ids = Project.objects.filter(env_id=env_id).first().merge_env_ids + ','.join(map(str, env_ids))
        else:
            new_env_ids = ','.join(map(str, env_ids))
