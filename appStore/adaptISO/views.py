...
class AdaptISOListViewSet(viewsets.ModelViewSet):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if request.user.is_superuser or request.user.chinese_name == iso_data.user_name:
            if iso_data.http_address != request.data.get('http_address'):
                ...
                if iso_data.ISO_name != new_iso_name:
                    if AdaptISO.objects.filter(ISO_name=new_iso_name):
                        return json_response({}, status.HTTP_400_BAD_REQUEST,'该ISO已经录入数据库，不可重复录入。')
            ...
            return json_response({}, status.HTTP_401_UNAUTHORIZED, '只有管理人员才能修改数据')
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if request.user.is_superuser or request.user.chinese_name == adapt_iso_data.user_name:
            ...
            return json_response({}, status.HTTP_401_UNAUTHORIZED, '只有管理人员才能删除该数据')
