...
class LmbenchViewSet(CusModelViewSet):
    ...
    def get_data(self, serializer_, datas, title_index, column_index, base_column_index):
        ...
        if len(serializer_) == 1:
            ...
            # 计算每个数组的平均值
            average_basic_Mhz = np.mean(basic_Mhz_list).round(2) if not basic_Mhz_list else None
            average_basic_tlb_pages = np.mean(basic_tlb_pages_list).round(2) if not basic_tlb_pages_list else None
            average_basic_cache_line_bytes = np.mean(basic_cache_line_bytes_list).round(2) if not basic_cache_line_bytes_list else None
            average_basic_mem_par = np.mean(basic_mem_par_list).round(2) if not basic_mem_par_list else None
            average_basic_scal_load = np.mean(basic_scal_load_list).round(2) if not basic_scal_load_list else None
            average_processor_null_call = np.mean(processor_null_call_list).round(2) if not processor_null_call_list else None
            average_processor_null_I_O = np.mean(processor_null_I_O_list).round(2) if not processor_null_I_O_list else None
            average_processor_stat = np.mean(processor_stat_list).round(2) if not processor_stat_list else None
            average_processor_open_clo = np.mean(processor_open_clo_list).round(2) if not processor_open_clo_list else None
            average_processor_slct_TCP = np.mean(processor_slct_TCP_list).round(2) if not processor_slct_TCP_list else None
            average_processor_sig_inst = np.mean(processor_sig_inst_list).round(2) if not processor_sig_inst_list else None
            average_processor_sig_hndl = np.mean(processor_sig_hndl_list).round(2) if not processor_sig_hndl_list else None
            average_processor_fork_proc = np.mean(processor_fork_proc_list).round(2) if not processor_fork_proc_list else None
            average_processor_exec_proc = np.mean(processor_exec_proc_list).round(2) if not processor_exec_proc_list else None
            average_processor_sh_proc = np.mean(processor_sh_proc_list).round(2) if not processor_sh_proc_list else None
            average_processor_Mhz = np.mean(processor_Mhz_list).round(2) if not processor_Mhz_list else None
            average_basic_intgr_bit = np.mean(basic_intgr_bit_list).round(2) if not basic_intgr_bit_list else None
            average_basic_intgr_add = np.mean(basic_intgr_add_list).round(2) if not basic_intgr_add_list else None
            average_basic_intgr_mul = np.mean(basic_intgr_mul_list).round(2) if not basic_intgr_mul_list else None
            average_basic_intgr_div = np.mean(basic_intgr_div_list).round(2) if not basic_intgr_div_list else None
            average_basic_intgr_mod = np.mean(basic_intgr_mod_list).round(2) if not basic_intgr_mod_list else None
            average_basic_int64_bit = np.mean(basic_int64_bit_list).round(2) if not basic_int64_bit_list else None
            average_basic_int64_add = np.mean(basic_int64_add_list).round(2) if not basic_int64_add_list else None
            average_basic_int64_mul = np.mean(basic_int64_mul_list).round(2) if not basic_int64_mul_list else None
            average_basic_int64_div = np.mean(basic_int64_div_list).round(2) if not basic_int64_div_list else None
            average_basic_int64_mod = np.mean(basic_int64_mod_list).round(2) if not basic_int64_mod_list else None
            average_basic_float_add = np.mean(basic_float_add_list).round(2) if not basic_float_add_list else None
            average_basic_float_mul = np.mean(basic_float_mul_list).round(2) if not basic_float_mul_list else None
            average_basic_float_div = np.mean(basic_float_div_list).round(2) if not basic_float_div_list else None
            average_basic_float_bogo = np.mean(basic_float_bogo_list).round(2) if not basic_float_bogo_list else None
            average_basic_double_add = np.mean(basic_double_add_list).round(2) if not basic_double_add_list else None
            average_basic_double_mul = np.mean(basic_double_mul_list).round(2) if not basic_double_mul_list else None
            average_basic_double_div = np.mean(basic_double_div_list).round(2) if not basic_double_div_list else None
            average_basic_double_bogo = np.mean(basic_double_bogo_list).round(2) if not basic_double_bogo_list else None
            average_context_2p_0K = np.mean(context_2p_0K_list).round(2) if not context_2p_0K_list else None
            average_context_2p_16K = np.mean(context_2p_16K_list).round(2) if not context_2p_16K_list else None
            average_context_2p_64K = np.mean(context_2p_64K_list).round(2) if not context_2p_64K_list else None
            average_context_8p_16K = np.mean(context_8p_16K_list).round(2) if not context_8p_16K_list else None
            average_context_8p_64K = np.mean(context_8p_64K_list).round(2) if not context_8p_64K_list else None
            average_context_16p_16K = np.mean(context_16p_16K_list).round(2) if not context_16p_16K_list else None
            average_context_16p_64K = np.mean(context_16p_64K_list).round(2) if not context_16p_64K_list else None
            average_local_2p_0K = np.mean(local_2p_0K_list).round(2) if not local_2p_0K_list else None
            average_local_Pipe = np.mean(local_Pipe_list).round(2) if not local_Pipe_list else None
            average_local_AF_UNIX = np.mean(local_AF_UNIX_list).round(2) if not local_AF_UNIX_list else None
            average_local_UDP = np.mean(local_UDP_list).round(2) if not local_UDP_list else None
            average_local_TCP = np.mean(local_TCP_list).round(2) if not local_TCP_list else None
            average_local_TCP_conn = np.mean(local_TCP_conn_list).round(2) if not local_TCP_conn_list else None
            average_local_RPC_TCP = np.mean(local_RPC_TCP_list).round(2) if not local_RPC_TCP_list else None
            average_local_RPC_UDP = np.mean(local_RPC_UDP_list).round(2) if not local_RPC_UDP_list else None
            average_local_bigger_Mmap_Latency = np.mean(local_bigger_Mmap_Latency_list).round(2) if not local_bigger_Mmap_Latency_list else None
            average_local_bigger_Prot_Fault = np.mean(local_bigger_Prot_Fault_list).round(2) if not local_bigger_Prot_Fault_list else None
            average_local_bigger_Page_Fault = np.mean(local_bigger_Page_Fault_list).round(2) if not local_bigger_Page_Fault_list else None
            average_local_bigger_100fd_selct = np.mean(local_bigger_100fd_selct_list).round(2) if not local_bigger_100fd_selct_list else None
            average_local_bigger_0K_File_create = np.mean(local_bigger_0K_File_create_list).round(2) if not local_bigger_0K_File_create_list else None
            average_local_bigger_0K_File_delete = np.mean(local_bigger_0K_File_delete_list).round(2) if not local_bigger_0K_File_delete_list else None
            average_local_bigger_10K_File_create = np.mean(local_bigger_10K_File_create_list).round(2) if not local_bigger_10K_File_create_list else None
            average_local_bigger_10K_File_delete = np.mean(local_bigger_10K_File_delete_list).round(2) if not local_bigger_10K_File_delete_list else None
            average_local_bigger_Pipe = np.mean(local_bigger_Pipe_list).round(2) if not local_bigger_Pipe_list else None
            average_local_bigger_AF_UNIX = np.mean(local_bigger_AF_UNIX_list).round(2) if not local_bigger_AF_UNIX_list else None
            average_local_bigger_TCP = np.mean(local_bigger_TCP_list).round(2) if not local_bigger_TCP_list else None
            average_local_bigger_File_reread = np.mean(local_bigger_File_reread_list).round(2) if not local_bigger_File_reread_list else None
            average_local_bigger_Mmap_reread = np.mean(local_bigger_Mmap_reread_list).round(2) if not local_bigger_Mmap_reread_list else None
            average_local_bigger_Bcopy_libc = np.mean(local_bigger_Bcopy_libc_list).round(2) if not local_bigger_Bcopy_libc_list else None
            average_local_bigger_Bcopy_hand = np.mean(local_bigger_Bcopy_hand_list).round(2) if not local_bigger_Bcopy_hand_list else None
            average_local_bigger_Mem_read = np.mean(local_bigger_Mem_read_list).round(2) if not local_bigger_Mem_read_list else None
            average_local_bigger_Mem_write = np.mean(local_bigger_Mem_write_list).round(2) if not local_bigger_Mem_write_list else None
            average_memory_Mhz = np.mean(memory_Mhz_list).round(2) if not memory_Mhz_list else None
            average_memory_L1 = np.mean(memory_L1_list).round(2) if not memory_L1_list else None
            average_memory_L2 = np.mean(memory_L2_list).round(2) if not memory_L2_list else None
            average_memory_Main_mem = np.mean(memory_Main_mem_list).round(2) if not memory_Main_mem_list else None
            average_memory_Rand_mem = np.mean(memory_Rand_mem_list).round(2) if not memory_Rand_mem_list else None
            # 基准数据和对比数据的全部数据
