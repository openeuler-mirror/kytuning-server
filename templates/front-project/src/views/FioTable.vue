  ...
  <div style="overflow-x: auto;">
    <el-table :data="other_list" border :span-method="titleObjectSpanMethod" style="overflow-y: auto;"
      ...
      <template v-for="i in numColumns" :key="i">
        <el-table-column :prop="`column${i}`" :width="i < 2 ? '150' : ''" align="center"></el-table-column>
      </template>
    </el-table>
  </div>
  <div style="overflow-x: auto;">
...
export default {
  ...
  created() {
    axios.get('/api/fio/?env_id=' + this.$route.params.baseId + '&comparsionIds=' + this.$route.params.comparsionIds).then((response) => {
