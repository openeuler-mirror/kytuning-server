<template>
  <div class="tableHome">
    <template v-if="isAllDataLoaded">
      <Header :dataName="dataName" baseId="baseId" comparsionIds="comparsionIds" ref="headerComponent" @data-loaded="getHeadrData"/>
<!--      <Left :dataName="dataName" :baseId="baseId" :comparsionIds="comparsionIds" ref="leftComponent" @data-loaded="accessChildData"/>-->
      <el-container class="content">
        <el-container>
          ...
          <div v-for="comparsionId in comparsionIds" :key="comparsionId">
<!--            <Compar :dataName="dataName" :baseId="comparsionId" comparsionIds="comparsionIds" ref="comparComponent" @data-loaded="accessChildData"/>-->
            ...
            <template v-if="isComparDataLoaded">
...
export default {
  ...
  data() {
    return {
      ...
      isComparDataLoaded: false, // 标记上一个compar数据是否加载完成
    };
  },
  created() {
    this.isAllDataLoaded = true;
    this.isComparDataLoaded = true;
    // this.$emit('compar-data-updated', this.comparDatas); // 发送初始 comparDatas
  },
  methods: {
    getHeadrData(value) {
    ...
    getComparData(baseId, titleIndex_, comparDatas) {
      // const comparsionId = this.comparsionIds[index];
      // this.titleIndex = titleIndex_
      console.log(baseId, 111);
      console.log(titleIndex_, 222);
      console.log(comparDatas, 333);
      this.comparDatas[baseId] = comparDatas;
      console.log(this.comparDatas, 444);
    },
  }
};
</script>
